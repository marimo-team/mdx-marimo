...
describe("island styling contract", () => {
  ...
  it("uses one package class and an MDX host attribute", () => {
    const mdxNodes = readFileSync(join("src", "mdx", "nodes.ts"), "utf8");
    const browserRuntime = readFileSync(join("src", "browser", "island.ts"), "utf8");

    expect(mdxNodes).toContain('"marimo-island-host"');
    expect(mdxNodes).toContain('"data-marimo-host"');
