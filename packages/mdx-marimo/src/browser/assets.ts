import type { MarimoRuntimeAssets } from "../schema";
...
declare global {
  interface Window {
    ...
    __MARIMO_EXPORT_CONTEXT__?: MarimoRuntimeAssets["exportContext"];
  }
}
...
export async function ensureAssets(assets: MarimoRuntimeAssets, appId: string): Promise<void> {
  ensureMountConfig(assets);
  ensureExportContext(assets);
  ...
  if (firstInitializedAppId === undefined) {
    firstInitializedAppId = appId;
    return;
  }
  await Promise.all(modules.map((runtimeModule) => runtimeModule.initialize?.()));
}
...
function ensureExportContext(assets: MarimoRuntimeAssets): void {
  const context = assets.exportContext;
  if (context?.trusted !== true) return;
  if (
    window.__MARIMO_EXPORT_CONTEXT__?.trusted === true &&
    window.__MARIMO_EXPORT_CONTEXT__.notebookCode === context.notebookCode
  ) {
    return;
  }
  window.__MARIMO_EXPORT_CONTEXT__ = {
    trusted: true,
    ...(context.notebookCode === undefined ? {} : { notebookCode: context.notebookCode }),
  };
}
